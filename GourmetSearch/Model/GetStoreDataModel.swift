...
class GetStoreDataModel {
    ...
    func getStoreDataForMap(range: Int, latitude: Double, longitude: Double) async throws -> StoreData {
    ...
    func getStoreDataForCategory(latitude: Double, longitude: Double, category: String) async throws -> StoreData {
        let APIKey = APIKey.APIKey
        let url = URL(string: "https://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=\(APIKey)&lat=\(latitude)&lng=\(longitude)&range=5&keyword=\(category)&count=100&format=json")!
        let (data,_) = try await URLSession.shared.data(from: url)
        let decodedStoreData = try JSONDecoder().decode(StoreData.self, from: data)
        return decodedStoreData
    }
